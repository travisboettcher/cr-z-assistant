...
penalty is worse than garbled — see [R1](rulings.md#r1--the-hunger-penalty-is-a-starvation-threshold-not-a-hunger-tax). The
...
**A utility point assigned in the Planning Phase stands for the turn, and the book says so rather
than this app ruling it.** The timing is the whole of it. Utilities are generated and assigned at
**Planning Step 1** and last until the next Planning Phase (pg. 20, 67). The hunger penalty is
applied at **Management Step 2**, four steps later in the same turn, and it lowers Cooperation and
therefore the Utilities Score.

Both halves of the obvious argument are true and they do not conflict. The Score *is* live — pg. 22
follows the penalty with "remember to adjust survivors' skill scores accordingly for the turn" — and
the penalty does reduce what the **next** turn's Planning Step 1 can generate, because it runs until
the next Management Phase and Planning is inside that window. But the points already on the board
were generated by a community that could generate them, and the book provides no mechanism for
revoking an assigned point: utilities expire at the next Planning Phase and nowhere else.

So `utilitiesScore` prices the backing against the penalty as it stood at Planning Step 1, not as it
stands now. What was wrong was not the arithmetic but the retroaction: a legal 4 Power / 3 Water
assignment became illegal four steps after it was made, the base sheet read **POWER ASSIGNED 4 / 2 —
Over what the base generates**, and `backedPoints` dropped points in layout order without saying so
— which the next Advancement Phase then read as unsupplied facilities, so production, healing and XP
all fell for a decision the player made legally and was never told had been undone
([#169](https://github.com/travisboettcher/cr-z-assistant/issues/169)).
...
across all four phases under one set of numbers. The rulings this phase depends on are
[R1](rulings.md#r1--the-hunger-penalty-is-a-starvation-threshold-not-a-hunger-tax) (the hunger
penalty, and the amendment recording that its expiry is turn-based rather than a ruling at all),
[R2](rulings.md#r2--the-rot-check-target-has-no-floor) (the Rot check target),
[R3](rulings.md#r3--at-the-base-at-departures-means-the-whole-community) (Departures),
[R6](rulings.md#r6--a-set-of-restraints-works-once-per-turn) (Restraints),
[R7](rulings.md#r7--a-point-of-xp-handed-out-stays-handed-out) (XP already awarded),
[R8](rulings.md#r8--a-teacher-with-a-score-of-nothing-still-replaces-the-discretionary-point) (the
Score-0 Teacher), [R9](rulings.md#r9--how-broadly-the-utilities-substitution-applies--open) (the
Utilities substitution, still open),
[R12](rulings.md#r12--wounded-and-injured-are-one-state) (wounded and injured), and
[R13](rulings.md#r13--siege-threat-may-go-below-zero--recorded-without-a-ruling) (negative Siege
Threat, recorded without a ruling).

This list numbered its own rulings 1–8 until the registry was written down. Two of them changed
number on the way: ruling 4 is now R9 and ruling 5 is now R12.
...
- Eligibility, from pp. 20–21: only survivors below maximum Health may rest or be healed; healing
  ...
  [R12](rulings.md#r12--wounded-and-injured-are-one-state).
...
- Materials: one d10 **per material recovered on the mission**, entered as rolled — no materials
  ...
  **changes a roll's result** rather than adding to it, with uses equal to that skill's summed
  ...
  [R9](rulings.md#r9--how-broadly-the-utilities-substitution-applies--open).
...
- **Check for Rot** (pg. 22): per 0-HP survivor, a Tier check against 12 − the combined Medicine
  ...
  [R2](rulings.md#r2--the-rot-check-target-has-no-floor).
...
- **The hunger penalty**: a shortfall drops stats, and therefore every Skill Score, until the
  ...
  [R1](rulings.md#r1--the-hunger-penalty-is-a-starvation-threshold-not-a-hunger-tax)** — the printed sentence is self-contradictory, and
...
- **Departures** (pg. 23): Unrest + Siege Threat ≥ 10 and the lowest-Tier survivor leaves; a tie
  ...
  [R3](rulings.md#r3--at-the-base-at-departures-means-the-whole-community).

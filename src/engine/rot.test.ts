...
describe('rotTarget', () => {
   ...
   *
   * **Supplied**, which the Med Lab's seat now depends on: two points assigned
   * to the slot are not two points generated, and pg. 67 gives an entry whose
   * requirements are unmet no effect at all (#153).
   */
  it('sums two medics rather than taking the better of them', () => {
    const both = staffedWith(
      generatingUtilities(community([], {}, medLab()), 2, 'front-yard'),
      'garage',
      [medic(3), { ...medic(2), id: 'other', name: 'Ada Poole' }],
    );
  ...
  /**
   * **#153 and #170, and the ruling that closed them.** The same Clinic with
   * the same two medics, on a Med Lab whose Power and Water are *assigned* and
   * generated by nobody: the upgrade produces no effect, so there is no second
   * seat and the second medic's Medicine does not reach the pool.
   *
   * This is the case the two halves of the app used to answer differently —
   * the staffing control said "Takes 2" off the resolved occupant while this
   * pooled one medic off the raw one, and on a Hydroelectric Dam the mismatch
   * ran the other way.
   */
  it('gives a Med Lab on an unbacked point no second seat', () => {
    const unsupplied = staffedWith(community([], {}, medLab()), 'garage', [
      medic(3),
      { ...medic(2), id: 'other', name: 'Ada Poole' },
    ]);

    expect(rotTarget(unsupplied)).toBe(9);
  });
  ...
  it('has no floor, and will go below zero for a well-staffed Clinic', () => {
    ...
    const crowded = staffedWith(
      generatingUtilities(community([], {}, medLab()), 2, 'front-yard'),
      'garage',
      [medic(8), { ...medic(8), id: 'two', name: 'Two' }],
    );

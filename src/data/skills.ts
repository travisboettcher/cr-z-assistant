/**
 * Survivor stats and skills — rules as data (pg. 40–41).
 *
...
/** The four stats every survivor has (pg. 40). */
export const STATS = ['strength', 'dexterity', 'intelligence', 'cooperation'] as const;

export type Stat = (typeof STATS)[number];

/**
 * Every governed skill and the stat that governs it (pg. 40), in rulebook order.
 *
 ...
 * *level* (1–4), and the two are the same sentence on pg. 30. Making "the
...
/** Every survivor starts with Move 6 and Defense 6 (pg. 41). */
export const COMMON_SKILL_START_SCORE = 6;

/** Neither common skill can be raised past a Score of 8 (pg. 30). */
